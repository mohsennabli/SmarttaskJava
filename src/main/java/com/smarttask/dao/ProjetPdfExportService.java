...
public class ProjetPdfExportService {
    ...
    private static final PDFont HELVETICA_OBLIQUE = new PDType1Font(Standard14Fonts.FontName.HELVETICA_OBLIQUE);
    ...
    private void drawMetricCard(PdfCursor cursor, float x, float y, float width, float height, SummaryMetric metric) throws IOException {
        ...
        cursor.content.beginText();
        cursor.content.setFont(HELVETICA_BOLD, 10);
    ...
    private void drawProjectSection(PdfCursor cursor, Projet projet, List<Tache> taches) throws IOException {
        ...
        writeText(cursor, projet.getNom(), HELVETICA_BOLD, 18, MARGIN + 16f, sectionTop - 44f, new Color(29, 59, 83));
        writeText(cursor, formatStatus(projet.getStatut()), HELVETICA_BOLD, 10, PAGE_WIDTH - 120f, sectionTop - 40f, accent);
        writeWrappedText(cursor, projet.getDescription(), HELVETICA, 11, MARGIN + 16f, sectionTop - 62f, CONTENT_WIDTH - 32f, 13f, new Color(55, 65, 81));
        ...
        writeText(cursor, "Taches associees", HELVETICA_BOLD, 13, MARGIN + 16f, tasksStart, new Color(29, 59, 83));
        float taskY = tasksStart - 18f;

        if (taches.isEmpty()) {
            writeText(cursor, "Aucune tache associee a ce projet.", HELVETICA_OBLIQUE, 11, MARGIN + 20f, taskY, new Color(107, 114, 128));
            cursor.y = taskY - 22f;
            return;
        }
        ...
        for (Tache tache : taches) {
            ...
            writeWrappedText(cursor, line, HELVETICA, 10, MARGIN + 24f, taskY, CONTENT_WIDTH - 40f, 12f, new Color(55, 65, 81));
            taskY -= 14f;
        }
    ...
    private void drawBadge(PdfCursor cursor, float x, float y, float width, float height, String text, Color background, Color foreground) throws IOException {
        ...
        cursor.content.beginText();
        cursor.content.setFont(HELVETICA_BOLD, 9);
